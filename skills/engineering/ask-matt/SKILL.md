...
If you want to lead the questions first, start with **`$discuss-with-docs`**. The agent answers, recommends changes, and maintains a working design with agreed decisions separated from proposals and open questions. You decide when discussion is complete. The usual next step is `$grill-with-docs` against that document, checking gaps and updating the same document as answers settle. Start that transition only on your instruction; you can explicitly skip grilling and request `$to-spec`. Discussion completion alone does not mean every design question is settled.
...
- **Something's broken** → **`$diagnosing-bugs`**. For hard bugs and performance regressions, it confirms the failure with existing commands, tests, or manual steps, investigates the cause, and verifies the fix. New automated reproduction code and regression tests follow the shared testing policy; confirmed manual steps are sufficient when they reliably show the failure.
