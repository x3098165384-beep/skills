...
You or the calling workflow supplies the fixed point. Otherwise the skill asks for one. It checks the reference and reviewable changes, including new files, before spawning either reviewer.
...
Yes, when reviewing current work, including calls from `implement`. It compares tracked changes against the starting commit and reads in-scope untracked files separately. Ordinary committed-code reviews retain the comparison against `HEAD`.

## It's working if

- It checks the baseline and includes in-scope new files before deciding there is nothing to review.
