...
When supplied discussion notes, a plan, or a design document, it also keeps that document current. It starts from existing agreed decisions, examines gaps and assumptions, and writes newly agreed answers back into the same document. This is optional: standalone use needs no prior discussion or document.
...
It needs two other skills present: [grilling](https://aihero.dev/skills-grilling) supplies the interview, [domain-modeling](https://aihero.dev/skills-domain-modeling) supplies the glossary and ADR discipline. Installing `grill-with-docs` alone gets you a skill that does not work.
...
| Other agreed design answers | The supplied working design document, or the conversation when none was supplied |

`CONTEXT.md` remains a glossary, and ADRs still require all three conditions. Supply a working design document when you want the other answers retained on disk, including a document developed through `$discuss-with-docs`. Without one, those answers remain in the conversation; keep that conversation for [to-spec](https://aihero.dev/skills-to-spec).
...
That is the skill failing to load its two dependencies. An agent that does not pick up [grilling](https://aihero.dev/skills-grilling) and [domain-modeling](https://aihero.dev/skills-domain-modeling) guesses at what grilling means, and you get an undifferentiated question dump. Partial loading is the more confusing case: `grilling` loads, `domain-modeling` does not, and you get a good interview with no paper trail. It correlates with model and [effort](https://www.aihero.dev/ai-coding-dictionary/effort) level, and it is the most reported problem with this skill. If you suspect it, ask the agent directly which skills it loaded.

**Where did all my other decisions go?**
When you supply a working design document, agreed answers are written back into it during the interview. Without one, answers that do not belong in the glossary or an ADR remain in the conversation. Carry the document and conversation into [to-spec](https://aihero.dev/skills-to-spec), and check that precise constraints survived the synthesis.

**Should I discuss the proposal before being grilled?**

Use `$discuss-with-docs` when you want to lead the questions and develop the proposal first. Then point `$grill-with-docs` at its working document. The interview checks gaps, contradictions, edge cases, and assumptions; it reopens an agreed decision when evidence or a conflict warrants it and explains why. You still confirm shared understanding before the interview closes.
