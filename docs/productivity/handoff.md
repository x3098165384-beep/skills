...
`handoff` compacts the conversation you are in into a **handoff document**: one markdown file, saved under the current project's `docs/handoff/` directory, that a fresh [agent](https://www.aihero.dev/ai-coding-dictionary/agent) can read to pick the work up. The document stays with the project alongside its other documentation.
...
Look in `docs/handoff/` at the root of the project being handed off, even if you invoked the skill from a subdirectory. Outside a repository, it uses the current workspace. The directory is created when needed, and each document gets a unique descriptive name such as `2026-09-25-143000-api-migration.md`. The agent returns a link to the file. If you explicitly request another output path, that path takes precedence.

**Will it survive between sessions?**
The handoff is a regular project file, so clearing the system's temporary directory does not remove it. To use it on another machine or in another checkout, transfer or commit it along with the project files it references. Referenced artifacts still need to be accessible to the next agent.
